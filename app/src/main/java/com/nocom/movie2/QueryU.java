...
public final class QueryU {
    ...
    public static List<Movie_detail_Activity> fetchTrailerData(String requestUrl) throws JSONException {
    ...
    */
    ...
    private static List<Movie_detail_Activity> extractFeatureFromJson(String moviejson) throws JSONException {
        ...
        try {
            ...
            JSONObject TrailerJson = new JSONObject(moviejson);
            JSONArray results = TrailerJson.getJSONArray("results");

            for (int J=0; J<results.length(); J++) {

                JSONObject currentMovie = results.getJSONObject(J);
                String key= currentMovie.getString("key");
                Movie_detail_Activity movie = new Movie_detail_Activity(key);
                movies.add(movie);
